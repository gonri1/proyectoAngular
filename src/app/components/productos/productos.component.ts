...
export class ProductosComponent {
  ...
  categoria: string | null = '';  // Esta propiedad almacenará el valor de la categoría
  precio: number | null = 0; // Esta propiedad almacen
  arrayPrecios: number[] = [];
  arrayCarrito: any[] = [];

  // Funcion de relleno de un array con cantidades y productos

  sumarCantidad(cantidad: number, titulo: string) {
    Swal.fire({
      title: "Estas seguro de agregarlo al carro?",
      showDenyButton: true,
      ...
      if (result.isConfirmed) {
        ...
        this.arrayPrecios.push(cantidad);
        this.arrayCarrito.push(`${cantidad}€, ${titulo}`);
      } else if (result.isDenied) {
        Swal.fire("No agregado al carro", "", "info");
      }
    });
  }

  // Funcion que va metiendo los importes de los productos seleccionados en un array
  ...
  calcularTotal() {
    return this.arrayPrecios.reduce((a, b) => a + b, 0);
  }
  ...
  //Array con los productos, traidos del json
  ...
  //el constructor con ActivatedRoute permite acceder a los parámetros de la URL en este caso categoria y lo capturamos
